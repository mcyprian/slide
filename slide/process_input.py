    ...
    for rule in pred:
        ...
        if not (re.search("->", rule)):
            ...
            while(re.search("\\&", rule1)):
                ...
                if not len(eq) == 2:
                    raise InputError(
                        "Parsing %s: only equalities of the form x=y separated by & are allowed" %
                        rule)
                added = 0
        ...
        while(re.search("\\&", rule)):
            ...
            if "!=" in eq:
                ...
                if not (len(eq) == 2):
                    raise InputError(
                        "Only equalities between two variables are allowed in pure part")
                if eq[0] == alloc:
                    equal.append(eq[1])
                elif eq[1] == alloc:
                    equal.append(eq[0])
                    ...
                    raise InputError(
                        " only equalities between allocated variable and something are allowed")
            rule = re.sub("^\\&[^\\*\\&]*", "", rule)
...
def create_new_rule(al, pt, calls, equal, pred_params, new_equalities, new_calls):
    ...
    for x in new_equalities:
        ...
        if tmp == 1:
            if "nil" in x:
                raise InputError(
                    "ERROR: formal parameter of nonempty rule equal to nil, Not implemented")
    ...
    for x in preds.keys():
        ...
        for i in range(len(rules)):
            ...
            if al == "":
                if not(calls == [] and pt == []):
                    raise InputError(
                        "Something odd hapaned: the predicate %s contains an empty with nonempty calls" %
                        x)
                inline_pred = x
                inline_par = par
                inline_eq = equal
    ...
    while len(TODO):
        ...
        for (alloc, pointsto, calls, equal) in preds[p_name][1]:
            if var_name == alloc or var_name in equal:
                ...
                if prop_calls == -1:
                    raise InputError(
                        "ERROR: tracked parameter propagated as two independent variables, not implemented")
                if tracked == "nil":
                    # the case when tracked is equal to nil is handled separatelly
                    if not dlt:
                        raise InputError(
                            "ERROR: something odd hapaned. Tracking parameter equal to nil and dlt==0")
                    new_pointsto = []
                    for i in pointsto:
                        if var_name == i:
                    ...
                    for (prop_pred, prop_item, prop_var_num) in prop_calls:
                        ...
                        new_calls = change_call_item_pred(
                            prop_item, prefix, prop_var_num, dlt, new_calls)
                        if not(((prop_pred, prop_var_num, dlt) in DONE)
                               or ((prop_pred, prop_var_num, dlt) in TODO)):
                    ...
                    if new_params.count(var_name):
                        ...
                        raise InputError(
                            "ERROR: parameter %s occures in the rule %s  more then once or no allocation" %
                            (var_name, p_name))
                    new_pointsto = []
                    for i in pointsto:
                        if var_name == i:
                    ...
                    if not(((prop_pred, prop_num, 0) in DONE)
                           or ((prop_pred, prop_num, 0) in TODO)):
                        TODO.append((prop_pred, prop_num, 0))
                else:
                    raise InputError(
                        "ERROR: variable %s not propagated from the rule %s" %
                        (tracked, p_name))
            else:
...
def rename_conflicts_with_params(preds, pars):
    ...
    while unique_var_prefix(prefix, preds, params):
        prefix = prefix + "X"
    # no we will go through the systme of predicates and rename each variable
    # "v" from params to "prefix+v"
    for p in preds.keys():
        (par, rules) = preds[p]
        new_par = []
        for x in par:
        ...
        if re.search("^[^\*]*->", rootcall):
            ...
            if '&' in rhs:
                ...
                while(re.search("\\&", rest)):
                    eq = re.sub("^\\&([^\\*\\&]*).*$", "\\1", rest)
                    # only disequalities of the form alloc!=nil are allowed. Added just for
                    # the SMT-COMP
                    if "!=" in eq:
                        eq = re.split("!=", eq)
                        if (eq[0] == alloc and eq[1] == "nil") or (
                                eq[1] == alloc and eq[0] == "nil"):
                            pass
                        else:
                            not_equal.append((eq[0], eq[1]))
...
def make_aut(preds, top_calls, params, root_rule, empty_rule, tiles):
    ...
    if type == 2:
        ...
        (root_rule, params, emptyheap_eq) = join_core.join(
            preds, top_calls, emptyheap_eq, ex_params)
        ...
        for i in range(0, len(params)):
            # or (params[i]=="nil") # nil is allways handled as existentially quantified variable
            ex_quantif = (params[i] in ex_params)
